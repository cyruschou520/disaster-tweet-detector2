# ================================================================
# DISASTER TWEET AI DETECTOR - MOCK API ONLY (QUICK DEPLOY)
# ================================================================
...
def display_probability_bar(disaster_prob, non_disaster_prob):
    ...
    <div style="background: #f0f0f0; border-radius: 10px; padding: 5px; margin: 20px 0;">
        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
            <span style="color: #ff4444;">DISASTER: {disaster_percent:.1f}%</span>
            <span style="color: #00cc66;">NON-DISASTER: {non_disaster_percent:.1f}%</span>
        </div>
        <div style="height: 30px; border-radius: 5px; overflow: hidden; display: flex;">
            <div style="width: {disaster_percent}%; background: linear-gradient(90deg, #ff4444, #cc0000); color: white; text-align: center; line-height: 30px;">
                {disaster_percent:.1f}%
            </div>
            <div style="width: {non_disaster_percent}%; background: linear-gradient(90deg, #00cc66, #008844); color: white; text-align: center; line-height: 30px;">
...
def create_location_map(location, lat, lon):
    fig = go.Figure()
    
    fig.add_trace(go.Scattermapbox(
        lat=[lat], lon=[lon],
        mode='markers+text',
        marker=dict(size=20, color='red', symbol='marker'),
        text=[location], textposition="top center"
    ))
    
    fig.update_layout(
        mapbox=dict(style="open-street-map", center=dict(lat=lat, lon=lon), zoom=10),
...
def display_live_feed():
    ...
    if analyses:
        ...
        st.dataframe(pd.DataFrame(feed_data), use_container_width=True, height=300)
...
st.markdown(
    ...
    <div class="main-header">
        ...
        <p style="font-size: 1.2em;">Quick Deploy Version - Mock API Only</p>
        <div><span class="mock-badge">🔍 Mock API (Always Available)</span></div>
...
with input_col1:
    ...
    tweet = st.text_area("", height=100, placeholder="Example: Heavy rain in Kampar causing flash floods...", key=input_key, label_visibility="collapsed")
...
cols = st.columns(4)
examples = [
    ("📰 Real", "Heavy rain in Kampar causing flash floods. According to JPS, water levels rising."),
    ("🚨 Fake", "URGENT! BREAKING: MASSIVE 8.0 earthquake in Kuala Lumpur! Thousands DEAD! SHARE NOW! 😱"),
    ("🔄 Mixed", "URGENT! Flood in Johor! Water level 2 meters! Official source says evacuating."),
    ("📍 Location", "Landslide reported in Cameron Highlands - authorities responding")
]
                ...
                if location:
                    ...
                    try:
                        ...
                        if data:
                            ...
                            st.plotly_chart(create_location_map(location, lat, lon), use_container_width=True)
                    except:
                        pass
    else:
...
st.markdown(
    ...
    <div style="text-align: center; padding: 20px;">
        <p>🚀 Disaster Tweet AI Detector | Mock API Version<br>
        Session: {st.session_state["session_id"]}</p>
        <p style="color: #888; font-size: 0.8em;">✅ DEPLOYS IMMEDIATELY - No model downloads required</p>
    </div>
    ''',
    unsafe_allow_html=True
)
